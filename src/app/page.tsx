...
import {
  ...
  ChevronDown, Layers, GraduationCap
} from 'lucide-react';
...
function SummarizerSection() {
  ...
  return (
    <section id="summarizer" className="relative py-20 px-4" style={{ backgroundColor: '#0a0a0f' }}>
      <div className="section-divider mb-20" />
      <div className="max-w-5xl mx-auto">
        <SectionHeader icon={Sparkles} title="وحدة الأستاذ - المُلخِّص الذكي" subtitle="ارفع صفحات أو أبواب كاملة من الكتب ليقوم الذكاء الاصطناعي بتلخيصها واستخراج زبدة المطالب" />
        ...
        <div className="bg-[#0d1117]/80 border border-emerald-500/15 rounded-2xl mt-8 overflow-hidden backdrop-blur-xl shadow-lg shadow-black/20">
          ...
          <div className="border-t border-emerald-500/10 p-4 bg-[#0d1117]">
            <div className="flex gap-3">
              ...
              <button onClick={handleSend} disabled={loading || !input.trim()} className="btn-green p-3 rounded-xl text-white disabled:opacity-50 transition-all shrink-0"><Send size={18} /></button>
            </div>
          </div>
        </div>
      </div>
...
/* ===================================================================
   TEACHER FLOATING BUTTON + MODAL
   =================================================================== */

function TeacherFloatingButton() {
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 left-6 z-[55] flex items-center gap-2.5 px-5 py-3 rounded-2xl shadow-lg shadow-emerald-500/20 border transition-all duration-300"
        style={{
      ...
      >
        ...
        <span className="text-sm font-bold">الأستاذ</span>
      </motion.button>

      {/* Teacher Modal */}
      <AnimatePresence>
        {isOpen && <TeacherModal onClose={() => setIsOpen(false)} />}
      </AnimatePresence>
    </>
  );
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setHasMindMap(data.hasMindMap || false);
      } else {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col"
        style={{
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-gray-100 font-bold text-base">نظام الأستاذ — البحث العميق</h2>
              <p className="text-gray-500 text-[10px]">استفسارات ذكية مدعومة بالذكاء الاصطناعي</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-[#1a1a2e] text-gray-400 hover:text-gray-100 transition-all"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4" style={{ scrollbarWidth: 'thin' }}>
          ...
          <div className="mb-4">
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 rounded-xl bg-[#111827] border border-emerald-500/15 text-gray-100 text-sm resize-none input-glow focus:outline-none transition-all"
              style={{ direction: 'rtl' }}
              disabled={loading}
            />
            <div className="flex items-center justify-between mt-3">
              <span className="text-gray-600 text-[10px]">Ctrl+Enter للبحث</span>
              <button
                onClick={handleSubmit}
                disabled={loading || !question.trim()}
                className="btn-green px-5 py-2.5 rounded-xl text-white text-sm font-medium flex items-center gap-2 disabled:opacity-50 transition-all"
              >
                {loading ? (
          ...
          {loading && (
            ...
            >
              <div className="relative">
                ...
                <GraduationCap size={20} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-emerald-400" />
              </div>
              <span className="text-gray-300 text-sm font-medium">الأستاذ يبحث في المصادر...</span>
              <span className="text-gray-500 text-xs">جارٍ تحليل السؤال وجمع المعلومات</span>
            </motion.div>
          )}
          ...
          {!loading && answer && (
            ...
            >
              ...
              {hasMindMap && mermaid && (
                <div className="mb-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-gray-300 text-xs font-semibold">خريطة ذهنية</span>
                  </div>
              ...
              {/* Text Answer */}
              <div className="rounded-xl bg-[#111827]/80 border border-emerald-500/10 p-5">
                <div className="flex items-center gap-2 mb-3">
                  <div className="p-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                    <GraduationCap size={14} className="text-emerald-400" />
                  </div>
                  <span className="text-gray-300 text-xs font-semibold">الإجابة</span>
                </div>
                <div
                  className="text-gray-300 text-sm leading-loose prose prose-invert prose-sm max-w-none"
